...
| Neo4j 5.26 Community | Bolt (`neo4rs`, `src/neo4j.rs`) and the HTTP Query API (`POST /db/neo4j/query/v2`, `src/neo4j_http.rs`) | two backends, `neo4j` and `neo4j-http`, sharing Cypher, labels, batching and the `(:V {id})` index; heap 2G / page cache 2G / `db.memory.transaction.total.max` 1G in the 6 GiB container budget (`compose.yaml`; 3G + 3G summed to the whole budget and Docker OOM-killed the container on soc-LiveJournal1, see FABLE-TO-FABLE §23); `read_path = harness-native-cypher` |
