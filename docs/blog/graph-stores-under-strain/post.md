...
**Grust's in-process Rust stores now match Neo4j's reach in the strain benchmark: grust-memory, LanceDB and Turso, in both of its journal modes, are clean on all four core scenario families up to com-Orkut, 117 million edges, the largest graph in the set, level with Neo4j over Bolt. In every same-machine pair where both passed, grust-memory beats Neo4j on loading, hub fan-out, hot-node writes, cold start and tail latency, by 5× to 111× at the median; Turso's WAL store beats it on loading, hub fan-out, deep walks, cold start and tail latency. Neo4j's remaining advantage is memory, and it is now small: 4.9 GB at com-Orkut against 5.1 GB for LanceDB, 5.4 GB for grust-memory, 6.1 GB for Turso MVCC and 7.0 GB for Turso WAL, where two days earlier the Turso stores took 2.5 to 3 times Neo4j's footprint.**

A graph store can answer quickly and still fail the workload around that answer. It may run out of memory while loading the graph, lose an acknowledged write under contention, or accumulate a long queue when requests arrive independently of its response time. The benchmark's rule has not changed: qualify a store by the workload it completes correctly within a stated resource budget, then compare speed among the completed cases. What changed between September 13 and September 19 is which stores complete the largest workload, and what it costs them in memory.
...
The practical lesson holds: **a fast answer is useful only inside a workload whose correctness, durability and failure behavior you have also measured.** As of September 19, the in-process Rust stores meet that bar at the benchmark's largest scale and answer faster there; Neo4j meets it with slightly less memory. Its [results and receipts](https://adversari.al/graph/strain) can be inspected independently of the sibling [query](https://adversari.al/graph/queries) and [graph-algorithm](https://adversari.al/graph/algorithms) experiments; none contributes to a blended score. The [Graph tab](https://adversari.al/graph) indexes all three.
