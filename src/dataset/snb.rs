...
//! stays as the `id` property. `organisation` and `place` split into the
//! labels their `type` column names (Company, University; Continent,
//! Country, City), as the LSQB projected-FK layout does. Relationship types
//! are the file's verb in upper snake case (`KNOWS`, `HAS_CREATOR`), and
//! the multi-valued attribute files (`person_speaks_language`,
...
pub fn load(
    ...
    for (stem, path) in files.iter().filter(|(stem, _)| !is_node(stem)) {
        ...
        match parts.as_slice() {
            // person_speaks_language, person_email_emailaddress
            [entity, verb, attribute] if *verb == "speaks" || *verb == "email" => {
            ...
            [source, verb, target] => {
                ...
                load_edges(&mut builder, source, verb, target, path)?;
            }
            _ => continue,
        }
    }
...
/// `Person`, or the label a `type` column names for organisations and places.
    ...
    fn typed_nodes_edges_and_attributes() {
        ...
        for (stem, path) in &files {
            ...
            match parts.as_slice() {
                ...
                [s, v, t] => load_edges(&mut builder, s, v, t, path).unwrap(),
                _ => {}
            }
        }
        ...
        let person = graph
            ...
            .unwrap();
        assert_eq!(person.props.get("id"), Some(&Value::Int(933)));
        assert!(matches!(
